...
def save_log(Ticketnumber, log_data, status="ok"):
    ...
    folder = os.path.join(BASE_DIR, "logs", "CloseIncident_Test")
    ...
    try:
        ...
        for name, func in steps:
            ...
            try:
                ...
                elif "Check Task CloseIncident button not should exist in Active,Pending Status (Technicals layer) Step2" == name:
                    if "Failed" in result: raise Exception(result)
                    step_info["status"] = result
                    print(result)
                    step_info["layer"] = Ticket_layer(DRIVER)
                    log_data["steps"].append(step_info)
                elif "Check Task CloseIncident button not should exist in Active Status (Analysis layer) Step3" == name:
                    if "Failed" in result: raise Exception(result)
                    step_info["status"] = result
                    print(result)
                    step_info["layer"] = Ticket_layer(DRIVER)
                    log_data["steps"].append(step_info)
                elif "Check Task CloseIncident Resolved Status (Feedback layer) Step4" == name:
                    if "Failed" in result: raise Exception(result)
                    step_info["status"] = result
                    print(result)
                    step_info["layer"] = Ticket_layer(DRIVER)
