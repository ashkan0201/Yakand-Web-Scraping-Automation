...
def save_log(Ticketnumber, log_data, status="ok"):
    ...
    folder = os.path.join(BASE_DIR, "logs", "AssignToPreviousSg_Test")
...
def process(Ticketnumber):
    ...
    try:
        ...
        for name, func in steps:
            ...
            try:
                result = func()
                if "Check Task AssignToPreviousSg Active status (aduit layer) Step1" == name:
                ...
                elif "Check Task AssignToPreviousSg Active status (Technicals layer) Step2" == name:
                    if "Failed" in result: raise Exception(result)
                    step_info["status"] = result
                    print(result)
                    step_info["layer"] = Ticket_layer(DRIVER)
